...
<template>
  <article class="item-detail">
    <header class="item-detail__header">
      <span class="item-detail__mark" aria-hidden="true"><KeyRound :size="22" /></span>
      <div>
        <p>API Key</p>
        <h2>{{ item.name }}</h2>
      </div>
    ...
    <dl class="item-detail__fields">
      <div><dt>项目</dt><dd>{{ item.account }}</dd></div>
      <div><dt>密钥</dt><dd><SensitiveField label="API Key" :value="item.secret" @copy="showCopied" /></dd></div>
      <div>
        <dt>控制台</dt>
        <dd>
